function [line1, line2, line3, line4] = FindQuadFrame_HiRes( Piece, showResults )
...
for k=1:size(props,1)
    ...
    if props(k).Area > 10
        ...
        if (props(k).BoundingBox(3) > 2 && props(k).BoundingBox(4) > 2)
            ...
            % At least one dimension is big enough
            if (props(k).BoundingBox(3) > 4 || props(k).BoundingBox(4) > 4)
                regions(l) = props(k);
